...
export async function trainAndPredictLSTM(timeAndPriceData: any[], newTimestamp: number, epochs: number) {
  ...
  model.add(tf.layers.lstm({ inputShape: [1, 1], units: 20, returnSequences: false }));
