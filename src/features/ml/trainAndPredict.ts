...
export async function trainAndPredict(timeAndPriceData: any[], newTimestamp: number, epochs: number) {
  ...
  const normalizedPredictedPrice = model.predict(tf.tensor2d([normalizedNewTimestamp], [1, 1])) as tf.Tensor;
