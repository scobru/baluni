...
export async function rebalancePortfolio(
  ...
  for (const token of desiredTokens) {
    ...
    tokenValue = await getTokenValue(
      ...
      decimals,
      usdcAddress,
      String(config?.SELECTED_CHAINID),
    );
  ...
  for (const token of desiredTokens) {
    ...
    if (difference < 0 && Math.abs(difference) > config?.LIMIT) {
      ...
      const tokenPriceInUSDT: any = await fetchPrices(_token, String(config?.SELECTED_CHAINID)); // Ensure this returns a value
  ...
  for (let { token, amount } of tokensToBuy) {
    ...
    const [rsiResult, stochasticRSIResult] = await getRSI(tokenSymbol, config);
    const usdBalance = (
      await getTokenBalance(dexWallet.walletProvider, dexWallet.walletAddress, USDC[config?.SELECTED_CHAINID])
    ).balance;
